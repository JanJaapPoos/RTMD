# Explicit handling for multiple locations with multiple timestamps.

tmd_predict <- function(filename, lat, lon, time, ptype = "h", ..., constituents = NULL, coasts = "nan", InferMinor = NULL, input_mode = c("paired", "grid"), time_mode = c("all", "track")) {
  ...
  input_grid_size <- if (is.null(dim(lat))) c(length(lat)) else dim(lat)
  input_time_size <- if (is.null(dim(time_num))) c(length(time_num)) else dim(time_num)
  lat_vec <- as.vector(lat)
  time_vec <- as.vector(time_num)

  if (time_mode == "track" && length(lat_vec) > 1 && length(time_vec) > 1 && length(lat_vec) != length(time_vec)) {
    stop("For time_mode = 'track', the number of locations must match the number of times.", call. = FALSE)
  }

  map_solution <- FALSE
  if (length(lat_vec) > 1 && length(time_vec) > 1) {
    map_solution <- identical(time_mode, "all")
  } else if (length(lat_vec) > 1 && length(time_vec) == 1) {
    map_solution <- FALSE
  }
  ...
  if (length(input_grid_size) == 1 && input_grid_size[1] == 1) {
  ...
  if (length(input_grid_size) == 1) {
    if (trailing_n <= 1) {
      return(array(z, dim = input_grid_size))
    }
    return(matrix(z, nrow = input_grid_size[1], ncol = trailing_n))
  }
  ...
  array(z, dim = c(input_grid_size, trailing_n))
}
